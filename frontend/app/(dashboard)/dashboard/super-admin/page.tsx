    ...
    <div className="min-h-full bg-[#F5F5F0]">
      <div className="border-b-4 border-black bg-white px-6 py-6 md:px-10">
        ...
        <div className="mt-2 flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
          ...
          <div className="border-4 border-black bg-[#FFD93D]/10 px-4 py-3">
            ...
            <p className="text-xs font-medium text-[#000000]/55">{appAdmin.email}</p>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-8 px-6 py-8 md:px-10">
        <div className="grid gap-4 md:grid-cols-3">
          <section className="border-4 border-black bg-white p-5 shadow-[8px_8px_0px_0px_#000]">
            <div className="flex items-center gap-3">
              <ShieldCheck className="h-5 w-5 text-[#FF6B6B]" strokeWidth={2.5} />
              <div>
                <p className="text-[10px] font-black uppercase tracking-[0.25em] text-[#000000]/45">
                  Admin Count
                </p>
                <p className="text-2xl font-black text-black">{adminCount}</p>
              </div>
            </div>
          ...
          <Link
            ...
            className="border-4 border-black bg-white p-5 shadow-[8px_8px_0px_0px_#FF6B6B] transition-transform hover:-translate-y-1"
          >
            <div className="flex items-center gap-3">
              <UsersRound className="h-5 w-5 text-[#FF6B6B]" strokeWidth={2.5} />
              <div>
                <p className="text-[10px] font-black uppercase tracking-[0.25em] text-[#000000]/45">
                  Customers
                </p>
                <p className="text-2xl font-black text-black">{customerCount}</p>
              </div>
            </div>
            <p className="mt-4 text-sm font-medium leading-6 text-black/60">
              Browse every account, inspect usage, and edit limits from one place.
            </p>
            <p className="mt-4 inline-flex items-center gap-2 text-xs font-black uppercase tracking-[0.18em] text-[#FF6B6B]">
          ...
          <Link
            ...
            className="border-4 border-black bg-white p-5 shadow-[8px_8px_0px_0px_#FFD93D] transition-transform hover:-translate-y-1"
          >
            <div className="flex items-center gap-3">
              <Workflow className="h-5 w-5 text-[#FF6B6B]" strokeWidth={2.5} />
              <div>
                <p className="text-[10px] font-black uppercase tracking-[0.25em] text-[#000000]/45">
                  Pending Approvals
                </p>
        ...
        <div className="grid gap-6 xl:grid-cols-[1.15fr_0.85fr]">
          <section className="border-4 border-black bg-white shadow-[8px_8px_0px_0px_#FFD93D]">
            ...
            {attentionAccounts.length === 0 ? (
              <div className="px-5 py-10 text-center">
                <p className="text-xs font-black uppercase tracking-[0.25em] text-[#000000]/35">
                  No urgent customer states right now
                </p>
              </div>
            ) : (
              <div className="divide-y divide-[#000000]/8">
                    ...
                    >
                      <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {overQuota && (
                            <span className="inline-flex items-center border-2 border-[#FF6B6B] bg-[#FF6B6B] px-2 py-1 text-[10px] font-black uppercase tracking-[0.18em] text-white">
                              Over Quota
                            </span>
                          )}
                          {!customer.automation_enabled && (
                          ...
                          {!customer.excel_export_enabled && (
                            <span className="inline-flex items-center border-4 border-black bg-[#FFD93D] px-2 py-1 text-[10px] font-black uppercase tracking-[0.18em] text-black">
                              Exports Off
                            </span>
                          )}
                          {customer.pending_platform_requests > 0 && (
          ...
          <section className="border-4 border-black bg-white shadow-[8px_8px_0px_0px_#000]">
            <div className="border-b-2 border-black px-5 py-4">
              <div className="flex items-center gap-3">
                <FlaskConical className="h-5 w-5 text-[#FF6B6B]" strokeWidth={2.5} />
                <div>
                  <p className="text-[10px] font-black uppercase tracking-[0.25em] text-[#000000]/45">
                    Audit Pipeline
                  </p>
            ...
            <div className="space-y-4 px-5 py-5">
              ...
              <form action={createSuperAdminAuditTestEntry}>
                <button
                  type="submit"
                  className="inline-flex min-h-[48px] items-center gap-2 border-4 border-black bg-black px-4 py-3 text-xs font-black uppercase tracking-[0.2em] text-white transition-colors hover:bg-[#FF6B6B]"
                >
        ...
        <section className="border-4 border-black bg-white shadow-[8px_8px_0px_0px_#FFD93D]">
          <div className="border-b-2 border-black px-5 py-4">
            <div className="flex items-center gap-3">
              <ScrollText className="h-5 w-5 text-[#FF6B6B]" strokeWidth={2.5} />
              <div>
                <p className="text-[10px] font-black uppercase tracking-[0.25em] text-[#000000]/45">
                  Recent Audit Logs
                </p>
              ...
              <table className="min-w-full divide-y-2 divide-[#000000]/10">
                ...
                <tbody className="divide-y divide-[#000000]/8">
                  {recentAuditLogs.map((log) => (
                    <tr key={log.id}>
                      <td className="whitespace-nowrap px-5 py-4 text-sm font-medium text-[#000000]/65">
                        {formatDate(log.created_at)}
                      </td>
                      <td className="px-5 py-4 text-sm font-black uppercase tracking-[0.15em] text-black">
                        {log.action}
                      </td>
                      <td className="px-5 py-4 text-sm font-medium text-[#000000]/65">
                        {log.target_type}
                      </td>
                      <td className="px-5 py-4 font-mono text-xs text-[#000000]/55">
