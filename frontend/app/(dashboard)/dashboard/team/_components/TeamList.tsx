  ...
  return (
    <>
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <Button variant="yellow" onClick={() => setShowCreate(true)}>
      ...
      {subAccounts.length === 0 && (
        <div className="border-4 border-dashed border-black p-16 text-center">
          <p className="font-black uppercase tracking-tighter text-black/30 text-lg">No team members</p>
                ...
                return (
                  ...
                  >
                    <td className="px-5 py-3">
                      <span className="text-sm font-black text-black">{sub.name}</span>
                    </td>
                    <td className="px-5 py-3">
                      <span className="text-sm font-medium text-black/60">{sub.email}</span>
                    </td>
                    <td className="px-5 py-3">
                      {viewCount === 0 ? (
                        <span className="text-xs font-bold uppercase tracking-widest text-black/30">No access</span>
                      ) : (
                        <div className="flex items-center gap-2">
                          <span className="flex items-center gap-1 text-xs font-bold uppercase tracking-widest text-[#FF6B6B]">
                            <Eye className="w-3 h-3" strokeWidth={2.5} />
                            {viewCount} bot{viewCount !== 1 ? 's' : ''}
                          </span>
                          {editCount > 0 && (
                            <span className="flex items-center gap-1 text-xs font-bold uppercase tracking-widest text-black">
                    ...
                    <td className="px-5 py-3">
                      <span className="text-xs font-medium text-black/40">{formatDate(sub.created_at)}</span>
                    </td>
                    <td className="px-5 py-3">
                      <div className="flex items-center justify-center gap-2">
                        <button
                          onClick={() => setManaging(sub)}
                          className="flex items-center gap-1 text-xs font-bold uppercase tracking-widest text-[#FF6B6B] border-2 border-[#FF6B6B] px-2 py-1 hover:bg-[#FF6B6B] hover:text-white transition-colors"
                        >
                        ...
                        <button
                          ...
                          className="p-1.5 border-2 border-[#FF6B6B] text-[#FF6B6B] hover:bg-[#FF6B6B] hover:text-white transition-colors disabled:opacity-40"
                        >
