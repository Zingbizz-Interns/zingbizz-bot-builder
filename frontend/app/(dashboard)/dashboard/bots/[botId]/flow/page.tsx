...
export default async function FlowPage({ params }: { params: Promise<{ botId: string }> }) {
  ...
  return (
    <div className="p-8">
      <div className="mb-6">
        <h2 className="text-3xl font-black uppercase tracking-tighter text-[#121212]">Flow Map</h2>
      ...
      <div className="flex items-center gap-5 mb-6">
        {[
          { type: 'Replier', color: '#1040C0' },
          { type: 'Form', color: '#D02020' },
          { type: 'Query', color: '#F0C020' },
        ].map(({ type, color }) => (
          <div key={type} className="flex items-center gap-2">
            <div className="w-4 h-4 border-2 border-[#121212]" style={{ backgroundColor: color }} />
            <span className="text-xs font-bold uppercase tracking-widest text-[#121212]/60">{type}</span>
          </div>
        ))}
        <span className="text-xs font-medium text-[#121212]/30 ml-2">
